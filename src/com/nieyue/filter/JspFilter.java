...
public class JspFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response,
        ...
        String remoteHost=servletRequest.getRemoteHost();						// 127.0.0.1					127.0.0.1
        ...
        if(rpath.startsWith("/resources")&&new File(strServletUrl+rpath).exists()){
			...
			if(urlpath4.indexOf("woman")>-1){
				servletRequest.getRequestDispatcher(cpath+"/woman/index.html").forward(request, response);
				return;
			}
//				if(urlpath4.indexOf("man")>-1){
//					servletRequest.getRequestDispatcher(cpath+"/man/index.html").forward(request, response);
//				return;
//				}
					servletRequest.getRequestDispatcher(cpath+"/dailyRecommended/index.html").forward(request, response);
